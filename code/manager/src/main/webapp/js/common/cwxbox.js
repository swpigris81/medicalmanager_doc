...
//			wd.style.zIndex = 9999999;
//			wd.style.cssFloat = "left";
		},
		scrolls:function(){
			var h = (cwxbox.page.height() - oh) /2;
			wd.style.top=(h+cwxbox.page.top())+'px';
 ...
 * @param msg 需要提示的信息
 */
function cwxAlert(msg, callbackFunc){
	var neirong = msg + '<br><input type=\'button\' value=\'确定\' class="cwxalert" onclick=\'javascript:handlerAlert()\'/>';
	this.handlerAlert = function(){
		cwxbox.box.hide();
 ...
 * 替换普通的confirm
 * @param msg 提示信息
 * @param handerOk 确定按钮处理事件
 * @param handerCancel 取消按钮处理事件
 * @returns
 */
function cwxConfirm(msg, handerOk, handerCancel){
	var neirong = msg + '<br><input type="button" value="确定" class="cwxalert" onclick="javascript:handler(\'1\')"/><input type="button" class="cwxcancel" value="取消" onclick="javascript:handler(\'0\')"/>';
	this.handler = function(o){
		if(o == "1"){
			
			if(handerOk && typeof(handerOk) == "function"){
				handerOk();
			}
			//cwxbox.box.hide();
		}else if(o == "0"){
			cwxbox.box.hide();
			if(handerCancel && typeof(handerCancel) == "function"){
				handerCancel();
			}
		}else{
			cwxbox.box.hide();
